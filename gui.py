...
def generate_file():
    ...
    messagebox.showinfo("Success", "The config.txt file has been generated.\n\nPlease click on the [Make 3D-print file] button after closing this window")
...
def run_main(): 
    try: 
        ...
        messagebox.showinfo("Success", "You have now a file named: plot_3D.eps on your computer.") 
        root.quit()  # Close the main window after execution
...
instruction_label = tk.Label(root, text="Please add your desired date and name in the boxes below.")
instruction_label.grid(row=0, column=0, columnspan=2, padx=10, pady=10)

# Create and place the date label and entry
tk.Label(root, text="Date (dd.mm.yyyy):").grid(row=1, column=0, padx=10, pady=10)
...
run_button = tk.Button(root, text="Make 3D-print file", command=run_main)
